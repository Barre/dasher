"""Fallback rule that probes __dasher_tokenize__ before raising.
...
def normalize_object_fallback(obj):
    method = getattr(obj, "__dasher_tokenize__", None)
    if method is not None and not isinstance(obj, type):
        return method()
    raise ValueError(
        f"No normalizer registered for {type(obj)!r} and no __dasher_tokenize__: {obj!r}"
    )
